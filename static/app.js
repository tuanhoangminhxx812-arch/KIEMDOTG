/* 
========================================================================
   HỆ THỐNG KIỂM DÒ TÀI KHOẢN TRUNG GIAN - CLIENT APP LOGIC
...
const state = {
    fileUploaded: false,
    hasAnalyzed: false,
    data: null, // Dữ liệu phân tích nhận được từ API
...
function initApp() {
    ...
    // Load trạng thái ứng dụng ban đầu từ Server
    checkServerState();
...
// Kiểm tra trạng thái hiện tại của server (nếu đã có file được tải lên trước đó)
async function checkServerState() {
    try {
        const response = await fetch('/api/state');
        const resData = await response.json();
        
        if (resData.original_filename) {
            state.fileUploaded = true;
            window.appElements.currentFilename.innerText = `File đang tải: ${resData.original_filename}`;
            window.appElements.btnAnalyze.disabled = false;
            window.appElements.connectionStatus.className = "status-indicator online";
            window.appElements.statusText.innerText = "Đã nhận file Excel";
            
            if (resData.has_analyzed) {
                await fetchAndDisplayData();
            }
        }
    } catch (err) {
        console.error("Không thể kết nối tới server:", err);
        showToast("Không thể kết nối tới máy chủ Flask!", "error");
    }
}
...
function registerEvents(el) {
    ...
    el.excelFileInput.addEventListener('change', async (e) => {
        ...
        el.statusText.innerText = "Đang tải file lên...";
        ...
        try {
            ...
            if (res.success) {
                ...
                el.currentFilename.innerText = `File đang tải: ${file.name}`;
                ...
                el.statusText.innerText = "Tải file thất bại";
                el.connectionStatus.className = "status-indicator offline";
                showToast(res.message, "error");
            }
        } catch (err) {
            console.error("Lỗi upload file:", err);
            showToast("Lỗi kết nối mạng khi tải file!", "error");
        } finally {
    ...
    el.btnAnalyze.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.success) {
                ...
                el.statusText.innerText = "Phân tích hoàn tất";
                showToast(res.message, "success");
            ...
            console.error("Lỗi khi phân tích đối chiếu:", err);
            el.loadingState.style.display = 'none';
            el.emptyState.style.display = 'flex';
            showToast("Lỗi trong quá trình phân tích đối chiếu chéo!", "error");
        } finally {
            el.loadingState.style.display = 'none';
            el.btnAnalyze.disabled = false;
            el.btnSelectExcel.disabled = false;
    ...
    el.btnExportFiltered.addEventListener('click', () => {
        ...
        let url = `/api/export_filtered?account=${state.selectedAccount}`;
        if (state.selectedCreator) {
            url += `&creator=${encodeURIComponent(state.selectedCreator)}`;
        }
        
        showToast("Đang chuẩn bị file Excel chi tiết lệch...", "success");
...
// Lấy dữ liệu phân tích hiện có trên server
async function fetchAndDisplayData() {
    try {
        ...
        console.error("Lỗi tải dữ liệu hiện tại:", err);
    }
}
